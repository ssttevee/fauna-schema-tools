...
pub fn generateTypescriptDefinitions(tree: fauna.SchemaTree) ?[]const u8 {
    return generateTypescriptDefinitionsInternal(tree) catch return null;
}
...
pub fn printCanonicalTree(tree: fauna.SchemaTree) ?[]const u8 {
    return printCanonicalTreeInternal(tree) catch return null;
}
...
pub fn linkFunctions(tree: fauna.SchemaTree) bool {
    linkFunctionsInternal(tree) catch return false;
    return true;
}
...
pub fn mergeRoles(tree: fauna.SchemaTree) ?fauna.SchemaTree {
    var copy = tree;
    mergeRolesInternal(&copy) catch return null;
    return copy;
}
...
pub fn parseSchemaTree(schema: []const u8) ?fauna.SchemaTree {
    return parseSchemaTreeInternal(schema) catch null;
}
